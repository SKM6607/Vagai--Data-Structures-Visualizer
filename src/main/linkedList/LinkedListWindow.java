...
final class LinkedListVisual extends LinkedList {
    ...
    public void insert(int value) {
        ...
        temp.setNextAddress(newNode.getAddress());
    ...
    public void drawNode(Graphics2D g, VisualNode node) {
        ...
        g.setColor(Color.WHITE);
        String nextAddr = node.isLast() ? "NULL" : node.getNextAddress().substring(0, Math.min(6, node.getNextAddress().length()));
        ...
        String shortAddr = node.getAddress().substring(0, Math.min(10, node.getAddress().length()));
...
public class LinkedListWindow extends JPanel implements DefaultWindowsInterface {
    private static final Font font = new Font(Font.SANS_SERIF, Font.BOLD, 20);
    private static LinkedListWindow singleton = null;
    private final JTextField textField;

    private LinkedListWindow() {
    ...
    public static LinkedListWindow createLinkedListWindow() {
        return (singleton == null) ? singleton = new LinkedListWindow() : singleton;
    }
