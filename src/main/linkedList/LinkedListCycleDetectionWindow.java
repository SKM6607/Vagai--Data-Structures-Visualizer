...
final class LinkedListCycleDetectionVisual extends LinkedList {
    ...
    LinkedListCycleDetectionVisual() {
    ...
    private void initializeSampleList() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (i > 0) {
                nodes.get(i - 1).setNextNode(node);
            }
            nodes.add(node);
        }
        resize();
    ...
    protected void drawInitialDetails(Graphics2D g) {
        drawTitle(g, "Linked List Cycle Detection", width / 2 - 250, 50);
    ...
    private void drawNodes(Graphics2D g) {
        ...
        for (VisualNode node : nodes) {
            if (node.getNextNode() != null) {
                ...
                int toX = node.getNextNode().xPos;
                int toY = node.getNextNode().yPos + nodeHeight / 2;
                // Check if this is a cycle connection
                if (nodes.indexOf(node.getNextNode()) <= nodes.indexOf(node)) {
    ...
    public void drawNode(Graphics2D g, VisualNode node) {
        ...
        g.drawString(node.getAddress().substring(0, Math.min(10, node.getAddress().length())), x + 5, y - 5);
    }

    private @NotNull Color getColor(VisualNode node) {
    ...
    public void appendNode(int value) {
        ...
        int x = nodes.isEmpty() ? 150 : nodes.getLast().xPos + nodeWidth + spacing;
        VisualNode newNode = new VisualNode(value, x, 200);

        if (!nodes.isEmpty()) {
            VisualNode last = nodes.getLast();
            // Remove any existing cycle first
            if (last.getNextNode() != null && nodes.indexOf(last.getNextNode()) < nodes.size() - 1) {
                last.setNextNode(null);
            }
            last.setNextNode(newNode);
            last.setNextAddress(newNode.getAddress());
        }
...
//        e(node,this);
    ...
    public void createCycle(int cycleToIndex) {
        ...
        VisualNode last = nodes.getLast();
        last.setNextNode(nodes.get(cycleToIndex));
        repaint();
    }
    ...
    public void detectCycleFloyd() {
        ...
        detectionResult = "";
        visitedNodes.clear();
        slowPointer = nodes.getFirst();
        fastPointer = nodes.getFirst();
        Timer timer = new Timer(animationSpeed, null);
        timer.addActionListener(e -> {
            // Move slow pointer one step
            if (slowPointer.getNextNode() != null) {
    ...
    private boolean movePointer(Timer timer) {
        if (fastPointer.getNextNode() != null) {
            fastPointer = fastPointer.getNextNode();
        } else {
            detectionResult = "NO CYCLE FOUND";
    ...
    public void detectCycleHashSet() {
        ...
        final VisualNode[] current = {nodes.getFirst()};

        Timer timer = getTimer(current);
        timer.start();
    }

    private @NotNull Timer getTimer(VisualNode[] current) {
        Timer timer = new Timer(animationSpeed, null);
        timer.addActionListener(e -> {
            ...
            current[0] = current[0].getNextNode();
            repaint();
        });
        return timer;
    }
...
public class LinkedListCycleDetectionWindow extends JPanel implements DefaultWindowsInterface {
    private static LinkedListCycleDetectionWindow singleton = null;
    private final LinkedListCycleDetectionVisual visualPanel;
    private final JTextField valueField;
    private final JTextField cycleField;
    private final Font font = new Font(Font.SANS_SERIF, Font.BOLD, 16);

    private LinkedListCycleDetectionWindow() {
        setLayout(new BorderLayout());
        visualPanel = new LinkedListCycleDetectionVisual();
    ...
    public static LinkedListCycleDetectionWindow createLinkedListCycleDetectionWindow() {
        return (singleton == null) ? singleton = new LinkedListCycleDetectionWindow() : singleton;
    }
    ...
    private JButton createButton(String text, Color bgColor) {
        return ComponentUtilities.createButton(text, bgColor, font);
    }
