...
final class CycleDetectionVisual extends LinkedList {
    ...
    public void detectCycleBrent() {
        ...
        Timer timer = getTimer(lambda, power);
        timer.start();
    }
    private @NotNull Timer getTimer(int[] lambda, int[] power) {
        Timer timer = new Timer(animationSpeed, null);
        timer.addActionListener(e -> {
            // Move fast pointer
            if (movePointer(timer)) return;
...
public class CycleDetectionWindow extends JPanel implements DefaultWindowsInterface {
    private static CycleDetectionWindow singleton = null;
    ...
    public static CycleDetectionWindow createLinkedListCycleDetectionWindow() {
        return (singleton == null) ? singleton = new CycleDetectionWindow() : singleton;
    }
